...
export const GET: RequestHandler = async ({ locals }) => {
    try {
        ...
        const response: Record<string, unknown> = {
            ...
            user: user ?? null,
            providers: user?.app_metadata?.provider ? [user.app_metadata.provider] : user?.identities?.map((i: any) => i.provider) ?? []
        };
        ...
        const providerToken = (await locals.supabase.auth.getSession()).data.session?.provider_token as string | undefined;
        ...
        if (providerToken && (response.providers as string[]).includes('google')) {
            try {
                ...
                const peopleRes = await fetch(`https://people.googleapis.com/v1/people/me?personFields=${encodeURIComponent(personFields)}`, {
