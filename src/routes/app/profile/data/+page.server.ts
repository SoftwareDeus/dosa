...
export const load: PageServerLoad = async (event) => {
	...
	// Legacy metadata fallback (avoid `undefined` in types by narrowing to null)
	const userMetadata = (user.user_metadata ?? null) as Record<string, Json> | null;
	const healthProfile = (userMetadata?.healthProfile ?? null) as Record<string, Json> | null;
	...
	return {
		...
		healthDefaults: {
			...
			heightCm: Number(tableHealth?.height_cm ?? healthProfile?.heightCm ?? 0),
			ageYears: Number(tableHealth?.age_years ?? healthProfile?.ageYears ?? 0),
			sex: (tableHealth?.sex ?? (healthProfile?.sex as string) ?? '') as string,
			weightKg: Number(tableHealth?.weight_kg ?? healthProfile?.weightKg ?? 0),
			dietType: (tableHealth?.diet_type ?? (healthProfile?.dietType as string) ?? '') as string,
			waterIntake: (tableHealth?.water_intake ??
				(healthProfile?.waterIntake as string) ??
				'') as string,
			allergies: (tableHealth?.allergies ?? (healthProfile?.allergies as string) ?? '') as string,
			medications: (tableHealth?.medications ??
				(healthProfile?.medications as string) ??
				'') as string,
			chronicConditions: (tableHealth?.chronic_conditions ??
				(healthProfile?.chronicConditions as string) ??
				'') as string,
			bloodType: (tableHealth?.blood_type ?? (healthProfile?.bloodType as string) ?? '') as string,
			activityLevel: (tableHealth?.activity_level ??
				(healthProfile?.activityLevel as string) ??
				'') as string
		}
	};
};
...
export const actions: Actions = {
	...
	saveHealth: async (event) => {
		...
		const heightVal = parseNumber(data.get('heightCm'));
		const heightCm =
