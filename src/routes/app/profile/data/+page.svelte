<script lang="ts">
    ...
    export let data: PageData & { me: any };

    let name = '';
    let phone = '';
    let avatarUrl = '';

    $: google = data.me?.data?.google;
    ...
    function back() { goto('/app/profile'); }
</script>
    ...
    <div class="p-6">
        <div class="mb-4 flex items-center justify-between">
            ...
            <div class="flex gap-2">
                <a href="/app/profile/data" class="rounded bg-gray-100 px-3 py-2 text-sm">{m.data_refresh()}</a>
                <button class="rounded bg-white px-3 py-2 text-sm border" on:click={back}>{m.data_back()}</button>
            </div>
        </div>
        ...
        <div class="grid gap-4 md:grid-cols-2">
            <section class="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
                ...
                <div class="space-y-2 text-sm">
                    ...
                    <div><span class="text-gray-500">Provider(s):</span> {data.me?.data?.providers?.join(', ')}</div>
                </div>
            </section>
            ...
            <section class="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
                ...
                {#if google}
                    <div class="space-y-2 text-sm">
                        ...
                        <div><span class="text-gray-500">Email:</span> {google?.emailAddresses?.[0]?.value}</div>
                        ...
                        <div><span class="text-gray-500">Birthday:</span> {google?.birthdays?.[0]?.text || google?.birthdays?.[0]?.date?.year + '-' + google?.birthdays?.[0]?.date?.month + '-' + google?.birthdays?.[0]?.date?.day}</div>
                        <div><span class="text-gray-500">Locale:</span> {google?.locales?.[0]?.value}</div>
                        <div><span class="text-gray-500">Organization:</span> {google?.organizations?.[0]?.name}</div>
