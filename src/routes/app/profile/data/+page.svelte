...
import { SEX_OPTIONS, DIET_OPTIONS, WATER_OPTIONS, ACTIVITY_OPTIONS, BLOOD_OPTIONS } from '$lib/types/health';
...
function labelSex(v: string): string {
    switch (v) {
        ...
        case 'prefer_not_to_say': return 'Keine Angabe';
        default: return v;
    }
}
...
async function saveHealthClient(event: SubmitEvent | MouseEvent): Promise<void> {
    try {
        ...
        console.error('saveHealthClient exception:', e);
        saveMessage = `Speichern fehlgeschlagen: ${e?.message || e}`;
        logDebug(`exception: ${e?.message || e}`);
    } finally {
        saveBusy = false;
        ...
        <form class="mt-8 max-w-2xl space-y-4" on:submit|preventDefault={saveHealthClient}>
            ...
            <div class="grid gap-4 md:grid-cols-2">
                ...
                <div>
                    ...
                    <input id="ageYears" name="ageYears" type="number" min="0" class="w-full rounded border px-2 py-1" bind:value={ageYears} />
                </div>
                <div>
                ...
                <div>
                    ...
                    <input id="weightKg" name="weightKg" type="number" min="0" step="0.1" class="w-full rounded border px-2 py-1" bind:value={weightKg} />
                </div>
                <div>
                ...
                <div>
                    <label class="mb-1 block text-sm text-gray-700" for="waterIntake">Wassertrinkverhalten</label>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="allergies" name="allergies" rows="2" class="w-full rounded border px-2 py-1" bind:value={allergies}></textarea>
                </div>
                <div class="md:col-span-2">
                    <label class="mb-1 block text-sm text-gray-700" for="medications">Medikamente</label>
                    <textarea id="medications" name="medications" rows="2" class="w-full rounded border px-2 py-1" bind:value={medications}></textarea>
                </div>
                <div class="md:col-span-2">
                    <label class="mb-1 block text-sm text-gray-700" for="chronicConditions">Chronische Erkrankungen</label>
                    <textarea id="chronicConditions" name="chronicConditions" rows="2" class="w-full rounded border px-2 py-1" bind:value={chronicConditions}></textarea>
                </div>
            </div>
            <div class="flex items-center gap-2">
                <button class="rounded bg-blue-600 px-3 py-2 text-sm text-white disabled:opacity-50" type="button" on:click={saveHealthClient} disabled={saveBusy}>
