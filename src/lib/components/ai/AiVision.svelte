...
<div class="min-h-screen bg-gray-50 p-4">
  <div class="mx-auto max-w-4xl space-y-6">
    ...
    {#if !selectedImage}
      <div class="rounded-xl bg-white p-6 shadow-sm">
        <div class="space-y-4">
          <div class="flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-gray-300 p-12 transition-colors hover:border-blue-500">
            ...
            <label class="mb-2 cursor-pointer rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-blue-700">
              Datei hochladen
              <input type="file" accept="image/*" onchange={handleFileUpload} class="hidden" />
            </label>
            <button onclick={capturePhoto} class="rounded-lg bg-gray-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-gray-700">
              📷 Foto aufnehmen
            </button>
          </div>
        </div>
      ...
      <div class="space-y-4">
        <div class="rounded-xl bg-white p-6 shadow-sm">
          <div class="mb-4 flex items-center justify-between">
            ...
            <button onclick={reset} class="text-sm text-red-600 hover:text-red-700">Zurücksetzen</button>
          </div>
          <img src={selectedImage} alt="Selected" class="w-full rounded-lg shadow-md" />
        </div>

        <div class="rounded-xl bg-white p-6 shadow-sm">
          <label for="prompt" class="mb-2 block text-sm font-medium text-gray-700">Was möchtest du über das Bild wissen?</label>
          <textarea id="prompt" bind:value={prompt} rows="3" class="w-full rounded-lg border border-gray-300 px-4 py-2 focus:border-blue-500 focus:ring-2 focus:ring-blue-500 focus:outline-none"></textarea>
          <button onclick={handleAnalyze} disabled={isLoading || !prompt.trim()} class="mt-4 w-full rounded-lg bg-blue-600 px-4 py-3 font-medium text-white transition-colors hover:bg-blue-700 disabled:bg-gray-300">
            {#if isLoading}
