...
<div class="flex h-full flex-col bg-gray-50">
    ...
    <div bind:this={chatContainer} class="flex-1 space-y-4 overflow-y-auto p-4">
        {#each messages as message, i (i)}
            <div class="flex {message.role === 'user' ? 'justify-end' : 'justify-start'}">
                <div class="max-w-[80%] rounded-2xl px-4 py-3 {message.role === 'user' ? 'bg-purple-600 text-white' : 'bg-white text-gray-900 shadow-sm'}">
                    <p class="text-sm leading-relaxed break-words whitespace-pre-wrap">
                        {message.content || (isLoading && i === messages.length - 1 ? '...' : '')}
                    </p>
                    {#if message.timestamp}
                        <p class="mt-1 text-xs {message.role === 'user' ? 'text-purple-200' : 'text-gray-400'}">{formatTime(message.timestamp)}</p>
                    {/if}
                </div>
            </div>
        {/each}
    ...
    <div class="border-t border-gray-200 bg-white p-4">
        <form onsubmit={handleSend} class="flex gap-2">
            <textarea bind:value={inputMessage} onkeydown={handleKeyDown} placeholder="Schreibe eine Nachricht..." disabled={isLoading} rows="1" class="flex-1 resize-none rounded-xl border border-gray-300 px-4 py-3 text-sm focus:border-purple-500 focus:ring-2 focus:ring-purple-500 focus:outline-none disabled:bg-gray-100 disabled:text-gray-500"></textarea>
            <button type="button" onclick={handleSend} disabled={isLoading || !inputMessage.trim()} class="flex h-12 w-12 items-center justify-center rounded-xl bg-purple-600 text-white transition-colors hover:bg-purple-700 disabled:bg-gray-300 disabled:text-gray-500">
                {#if isLoading}
